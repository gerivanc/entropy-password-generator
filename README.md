...
- Supports uppercase letters (A-Z), lowercase letters (a-z), digits (0-9), and special characters (!@#$%^&*()_+-=[]{}|;:,.<>?~`\\) — 29 symbols in total.
...
|------|--------|---------------|------------------|----------------|----------------|----------|
| 11 | 15 | Full (uppercase, lowercase, digits, symbols, no ambiguous) | 81 | 95.10 | Strong | Personal accounts (email, social media) |
...
| 12 | 18 | Full (uppercase, lowercase, digits, symbols, with ambiguous) | 91 | 117.14 | Very Strong | Professional accounts (work email, VPN) |
...
| 6 | 24 | Digits + Special (with ambiguous) | 39 | 126.85 | Very Strong | API tokens with limited character sets |
| 3 | 24 | Uppercase + Lowercase (with ambiguous) | 52 | 136.81 | Very Strong | General-purpose website logins |
| 1 | 24 | Lowercase + Special (with ambiguous) | 55 | 138.75 | Very Strong | Secure notes or backup codes |
| 2 | 24 | Uppercase + Special (with ambiguous) | 55 | 138.75 | Very Strong | Administrative console access |
| 7 | 24 | Uppercase + Lowercase + Digits (with ambiguous) | 62 | 142.90 | Very Strong | Multi-user system credentials |
| 9 | 24 | Uppercase + Digits + Special (with ambiguous) | 65 | 144.54 | Very Strong | Database access keys |
| 10 | 24 | Lowercase + Digits + Special (with ambiguous) | 65 | 144.54 | Very Strong | Secure file encryption |
| 8 | 24 | Uppercase + Lowercase + Special (with ambiguous) | 81 | 152.16 | Extremely Strong | High-security application logins |
| 15 | 24 | Full (uppercase, lowercase, digits, symbols, no ambiguous) | 81 | 152.16 | Extremely Strong | Enterprise-grade passwords |
| 16 | 32 | Full (uppercase, lowercase, digits, symbols, no ambiguous) | 81 | 202.88 | Cryptographic Grade | API keys for sensitive services |
| 17 | 42 | Full (uppercase, lowercase, digits, symbols, no ambiguous) | 81 | 266.27 | Cryptographic Grade | Server authentication tokens |
| 18 | 60 | Full (uppercase, lowercase, digits, symbols, no ambiguous) | 81 | 380.39 | Ultra Secure | Financial system credentials |
| 19 | 75 | Full (uppercase, lowercase, digits, symbols, no ambiguous) | 81 | 475.49 | Ultra Secure | Master keys for password managers |
| 20 | 128 | Full (uppercase, lowercase, digits, symbols, no ambiguous) | 81 | 811.50 | Ultra Secure (Theoretical Maximum) | Cryptographic keys, blockchain wallets |
...
|----------------|---------------------|
| `Mode 8 (152.16 bits)` | High-security website logins (e.g., cloud storage, VPNs) |
...
**All 20 modes produce passwords with entropies from 95.10 to 811.50 bits**, surpassing Proton© (75 bits) and NIST (80+ bits) standards. For example:
- Mode 11 (15 chars, full no ambiguous, \( R=81 \)): \( \log_2(81) \times 15 \approx 95.10 \) bits.
- Mode 20 (128 chars, full no ambiguous, \( R=81 \)): \( \log_2(81) \times 128 \approx 811.50 \) bits.
...
- Using `--length 15 --with-ambiguous` (15 chars, all types, with ambiguous, ( R=91 )): ( \log_2(91) \times 15 \approx 97.62 ) bits.
- Using `--length 128 --with-ambiguous` (128 chars, all types, with ambiguous, ( R=91 )): ( \log_2(91) \times 128 \approx 833.00 ) bits.
...
#### © 2025 EntroPy Password Generator - A secure and customizable password generator written in Python
